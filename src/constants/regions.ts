...
// ヘルパー関数
export const getRegionByCode = (code: number): Region | undefined => {
  return REGIONS.find((r) => r.code === code);
};

export const getPrefectureByCode = (code: number): Prefecture | undefined => {
  return PREFECTURES.find((p) => p.code === code);
};

export const getPrefecturesByRegion = (regionCode: number): Prefecture[] => {
  return PREFECTURES.filter((p) => p.regionCode === regionCode);
};

export const getTokyoWardByCode = (code: number): TokyoWard | undefined => {
  return TOKYO_WARDS.find((w) => w.code === code);
};

// 東京都のコード
export const TOKYO_PREFECTURE_CODE = 13;
